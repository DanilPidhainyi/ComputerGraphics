...
class GrowthParallelepiped(Parallelepiped):
    ...
    def draw(self, matrix, dye, random_colors):
        ...
        # малюємо ребра із заливкою від раста до раста
        for i in range(len(matrix)-1, -1, -1):
            draw_polygon(matrix[i], (matrix[i][0], matrix[i][1] + 100),
                         (matrix[i - 1][0], matrix[i - 1][1] + 100), matrix[i - 1],
                         contour_color=dye, fill_color=random_colors[i])

        # малюєм контур верхньої грані із заливкою від раста до раста
        draw_polygon(*matrix, contour_color=dye, fill_color=random_colors[-2])
